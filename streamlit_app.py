...
from zenml.client import Client  # <--- We use Client to fetch the model

def main():
    st.title("End to End Customer Satisfaction Pipeline with ZenML")

    # Load assets if they exist, otherwise skip to avoid errors
    try:
    ...
    if st.button("Predict"):
        with st.spinner("Loading model from latest pipeline run..."):
            try:
                # --- NEW LOGIC: Fetch model directly from artifact store ---
                client = Client()
                # Get the last successful run of the training pipeline
                last_run = client.get_pipeline("continous_deployment_pipeline").last_run
                
                # Fetch the 'trained_model' artifact from the 'model_train' step
                model = last_run.steps["model_train"].output.load()
                ...
                # Run Prediction directly using the loaded model object
                pred = model.predict(df)
                
                st.success(
                    "Your Customer Satisfactory rate (range between 0 - 5) is: {:.2f}".format(
                        pred[0]
                    )
                )
            except Exception as e:
                st.error(f"Error loading model or predicting: {e}")
                st.info("Make sure you have run the training pipeline first: `python run_local_deployment.py --config deploy`")
    ...
    if st.button("Results"):
        ...
        st.write(
            "Following figure shows how important each feature is in the model that contributes to the target variable."
        )
        try:
